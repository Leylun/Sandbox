...
class Travel_Tracker(App):
    ...
    def make_widget(self):
        # //Method to create/bind/colour/count(unvisited) buttons
        unvisited_places = 0
        if self.first_run:
            self.switch_sort()
    ...
    def switch_sort(self):
        # //Method for switching through sorting types ('name', 'country', 'priority', 'visited')
    ...
    def clear_inputs(self, clear_label):
        # //Method to clear inputs, loop for ease of improvement/addition
        if clear_label:
    ...
    def toggle_visited(self, given_name):
        # //Method to toggle visited status and display text outputs onto label (visited_label)
        for the_place in self.place_objects:
            if given_name == str(the_place):
                if the_place.visited:
                    the_place.visited = False
